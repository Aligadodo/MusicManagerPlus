...
import '../../widgets/preview/task_list_widget.dart';
import '../../widgets/preview/task_detail_widget.dart';
...
class _PreviewPageState extends ConsumerState<PreviewPage> {
  ...
  void _connectToTaskWebSocket(String taskId) {
    ...
    try {
      final channel = WebSocketChannel.connect(
        Uri.parse('ws://localhost:8080/ws/task/$taskId'),
      );
      ...
      channel.stream.listen(
        (message) {
          try {
            ...
            final taskStatus = task_models.TaskStatus.fromJson(data);
            ...
            if (mounted) {
              setState(() {
                ...
                if (_selectedTask?.taskId == taskId) {
                  _selectedTask = taskStatus;
                }
              });
            }

            if (['COMPLETED', 'FAILED', 'CANCELLED'].contains(taskStatus.status)) {
              _disconnectFromTaskWebSocket(taskId);
            }
          } catch (e) {
  ...
  void _disconnectFromTaskWebSocket(String taskId) {
    final channel = _webSocketChannels.remove(taskId);
    if (channel != null) {
      channel.sink.close();
    }
  }
