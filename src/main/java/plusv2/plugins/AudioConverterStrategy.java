...
public class AudioConverterStrategy extends AppStrategy {
    ...
    @Override public String getName() { return "音频格式转换 (高并发/CD修复)"; }
    @Override
    ...
    public void loadConfig(Properties props) {
        ...
        if (props.containsKey("ac_threads")) { try { spThreads.getValueFactory().setValue(Integer.parseInt(props.getProperty("ac_threads"))); } catch (Exception e) {} }
    ...
    private void runFFmpegJob(FFmpeg ffmpeg, File source, File target, Map<String, String> params, boolean mapMetadata) throws IOException {
        ...
        if (params.containsKey("sample_rate")) outputBuilder.setAudioSampleRate(Integer.parseInt(params.get("sample_rate")));
    ...
    public List<ChangeRecord> analyze(List<ChangeRecord> inputRecords, List<File> rootDirs, BiConsumer<Double, String> progressReporter) {
        ...
        return inputRecords.parallelStream().map(rec -> {
            ...
            if (fileExt.equals(extension) && mode[0] != null && mode[0].startsWith("原目录") && !isCdMode && !forceMeta) return rec;
            ...
            else if (mode[0].startsWith("子目录")) targetFile = new File(new File(parent, relPath == null || relPath.isEmpty() ? "converted" : relPath), newName);
            else if (mode[0].startsWith("同级目录")) targetFile = new File(new File(parent.getParentFile(), relPath == null || relPath.isEmpty() ? parent.getName() + "_" + extension : relPath), newName);
