...
public class MetadataHelper {
    ...
    private static boolean isValid(String s) { return s != null && !s.trim().isEmpty(); }
    private static boolean isMessy(String s) { if (s == null) return false; return s.contains("\uFFFD") || s.contains("????"); }
    ...
    public static AudioMeta extractFromFileSystem(File file) {
        ...
        if (name.matches("^\\d+[.\\s-].*")) {
            ...
            if (parts.length > 1) { meta.track = parts[0].trim(); meta.title = removeExt(parts[1].trim()); }
        } else if (name.contains(" - ")) {
            String[] parts = name.split(" - ");
            if (parts.length >= 2) { meta.artist = parts[0].trim(); meta.title = removeExt(parts[1].trim()); }
        ...
        if (parent != null) {
            ...
            if (parentName.matches("^\\d{4}\\s+-\\s+.*")) { meta.year = parentName.substring(0, 4); meta.album = parentName.substring(7).trim(); }
            else if (parentName.contains(" - ")) { String[] parts = parentName.split(" - ", 2); if (!isValid(meta.artist)) meta.artist = parts[0].trim(); meta.album = parts[1].trim(); }
            else { meta.album = parentName; }
    ...
    private static String removeExt(String s) { int d = s.lastIndexOf('.'); return d > 0 ? s.substring(0, d) : s; }
