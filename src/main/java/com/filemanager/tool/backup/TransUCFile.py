...
MP3_PATH = 'C:/Users/28667/Downloads/VipSongsDownload/'  # 存放歌曲路径
PROCESSED_FILES = []
PROCESSED_SONGS = []
class Transform():
    def do_transform(self):
        ...
        while True:
            try:
                ...
                for task in as_completed(all_task, timeout=60):
                    task.result()
                break
...
def process(file):
    ...
    if not file[-3:] == '.uc':  # 后缀uc结尾为歌曲缓存
        return 1
    # 跳过未缓存完毕的文件
    idxFileName = file.replace('.uc','.idx')
    if not os.path.exists(UC_PATH + idxFileName):
        return 0
    # 解析idx文件 判断是否加载全部数据
    try:
        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(current_time + "  ----- read idx file " + file + " begin")
        with open(UC_PATH + idxFileName +"") as f:
            # 使用json.load()函数加载文件内容并解析为JSON对象
            data = json.load(f)
            dataSize = data['size']
            dataZone = data['zone']
            if len(dataZone) != 1 or  dataZone[0].count(' ') > 1:
                print(current_time + "  ----- partly cached file " + file + " end")
                return 1
            if int(dataSize) != (int(dataZone[0][dataZone[0].rfind(' ')+1:])+1):
                print(current_time + "  ----- not full cached file " + file + " end")
                return 1
    except Exception as e:
        print("read idx "+file+" fail " + f"{e}")

    mp3_content = None
    # 缓存文件解密
    try:
        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(current_time + "  ----- read cache file " + file + " begin")
    ...
    # 文件名预处理
    current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(current_time + "  ----- read cache file " + file + " end~")
    ...
    song_name = song_name.replace("\\:", "：")
    current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    print(current_time + "  -----  " + song_name + " begin~")
    mp3_file_name = MP3_PATH + '%s.mp3' % (song_name)

    # 写入目标文件
    try:
        if os.path.exists(mp3_file_name) or mp3_file_name in PROCESSED_SONGS:
            current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
            print(current_time + "  -----  " + song_name + " exists~")
        else:
            with open(mp3_file_name+".temp", 'wb') as mp3_file:
                mp3_file.write(mp3_content)
            os.rename(mp3_file_name+".temp", mp3_file_name)
        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(current_time + "  -----  " + song_name + " done, (not)delete cache file;")
        # os.remove(UC_PATH+file)
        PROCESSED_FILES.append(file)
        PROCESSED_SONGS.append(mp3_file_name)
    except Exception as e:
        current_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(current_time + "  -----  " + song_name + " PROCESS FAILED!!!")
        print(e)
        return 1
