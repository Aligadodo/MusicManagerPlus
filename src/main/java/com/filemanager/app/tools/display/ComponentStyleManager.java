...
public class ComponentStyleManager {
     ...
     * 重新加载CSS文件
     */
    private static void reloadCssFiles() {
        // 这里可以添加重新加载CSS文件的逻辑
        // 例如，清除已加载的样式表并重新加载
    }
    ...
    private static void applyTabPaneStyle(TabPane tabPane) {
        ...
        // 从CSS文件加载样式
        loadStyleFromCssFile(tabPane, "/style/css/components.css");

        // 设置TabPane的基础样式
        tabPane.setStyle(String.format(
                "-fx-tab-min-height: %.1f; -fx-tab-max-height: %.1f; -fx-tab-min-width: %.1f; -fx-tab-max-width: %.1f;",
    ...
    /**
     * 应用TabPane子组件样式
     * @param tabPane TabPane组件
     * @param theme 主题配置
     * @param transparentBg 透明背景色
     * @param fontSize 字体大小
     */
    private static void applyTabPaneSubComponentStyles(TabPane tabPane, ThemeConfig theme, String transparentBg, double fontSize) {
        // 构建子组件样式
        String subComponentStyles = 
                // 2. 表头区域样式
                ...
                String.format(
                        ".tab-pane > .tab-header-area > .tab-header-background {\n" +
                                ...
                                "}\n",
                        transparentBg, theme.getBorderColor(), theme.getBorderWidth()
                ) +
                ...
                String.format(
                        ".tab-pane > .tab-header-area > .headers-region > .tab {\n" +
                                ...
                                "    -fx-cursor: hand;\n" +
                                "}\n",
                        transparentBg, theme.getBorderColor(), theme.getBorderWidth(), theme.getBorderWidth(), theme.getBorderWidth(), theme.getCornerRadius(), theme.getCornerRadius()
                ) +
                ...
                String.format(
                        ".tab-pane > .tab-header-area > .headers-region > .tab:selected {\n" +
                                ...
                                "    -fx-border-width: %.1f %.1f 0 %.1f;\n" +
                                "}\n",
                        transparentBg, theme.getAccentColor(), theme.getAccentColor(), transparentBg, theme.getAccentColor(), theme.getBorderWidth(), theme.getBorderWidth(), theme.getBorderWidth()
                ) +
                ...
                String.format(
                        ".tab-pane > .tab-content-area {\n" +
                                ...
                                "}",
                        transparentBg, theme.getBorderColor(), theme.getBorderWidth(), theme.getCornerRadius(), theme.getCornerRadius(), theme.getCornerRadius()
                );
        
        // 将样式添加到场景的样式表中
        if (tabPane.getScene() != null) {
            String styleSheetUrl = "data:text/css," + subComponentStyles;
            try {
                tabPane.getScene().getStylesheets().add(styleSheetUrl);
            } catch (Exception e) {
                // 忽略异常
            }
        }
    ...
    private static void applyTitledPaneStyle(TitledPane titledPane) {
        // 从CSS文件加载样式
        loadStyleFromCssFile(titledPane, "/style/css/components.css");
    }
    
    /**
     * 应用TitledPane子组件样式
     * @param titledPane TitledPane组件
     * @param theme 主题配置
     * @param transparentBg 透明背景色
     */
    private static void applyTitledPaneSubComponentStyles(TitledPane titledPane, ThemeConfig theme, String transparentBg) {
        // 构建子组件样式
        String subComponentStyles = String.format(
                ".titled-pane > .title {\n" +
                        ...
                        "}",
                transparentBg, theme.getTextPrimaryColor(), theme.getFontFamily(), theme.getFontSize(), theme.getBorderWidth(), theme.getBorderColor(),
                theme.getTextPrimaryColor(),
                transparentBg, theme.getCornerRadius(), theme.getCornerRadius()
        );
        ...
        if (titledPane.getScene() != null) {
            String styleSheetUrl = "data:text/css," + subComponentStyles;
            try {
                titledPane.getScene().getStylesheets().add(styleSheetUrl);
            } catch (Exception e) {
                // 忽略异常
            }
        }
    ...
    private static void loadStyleFromCssFile(Node node, String cssFilePath) {
        ...
        // 获取CSS文件的URL
        java.net.URL cssUrl = ComponentStyleManager.class.getResource(cssFilePath);
        if (cssUrl != null) {
    ...
    private static void applyComboBoxStyle(ComboBox<?> comboBox) {
        ...
        // 为面板背景添加透明度，实现玻璃效果
        String panelBgColor = "transparent";
        ...
        int openBraces = 0;
        int closeBraces = 0;
        for (char c : styleBuilder.toCharArray()) {
            if (c == '{') {
                openBraces++;
            } else if (c == '}') {
                closeBraces++;
        ...
        // 如果右花括号多于左花括号，移除多余的右花括号
        while (closeBraces > openBraces) {
            styleBuilder = styleBuilder.substring(0, styleBuilder.lastIndexOf('}')).trim();
            closeBraces--;
        }

        comboBox.setStyle(styleBuilder);
    }
    ...
    private static void applyCheckBoxStyle(CheckBox checkBox) {
        ...
        String styleBuilder = String.format(
                "%s -fx-text-fill: %s; -fx-font-family: '%s'; -fx-background-color: transparent;",
                trimmedStyle, textColor, theme.getFontFamily()
        );
        ...
        checkBox.setStyle(styleBuilder);
    }
    ...
    private static void applyRadioButtonStyle(RadioButton radioButton) {
        ...
        radioButton.setStyle(styleBuilder);
    }
    ...
    private static void applyToggleButtonStyle(ToggleButton toggleButton) {
        ...
        toggleButton.setStyle(styleBuilder);
    }
