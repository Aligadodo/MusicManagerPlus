...
**接口**: `DELETE /tasks/{taskId}/execution-logs`
...
4. **删除限制**: 预览中或执行中的任务默认不允许删除，只有停止状态的任务才能被删除
5. **批量删除**: 批量删除任务时会跳过正在执行的任务
6. **重新执行**: 重新执行任务会重置任务状态和进度
7. **WebSocket连接**: 建议在任务开始前建立WebSocket连接以接收实时更新
8. **分页参数**: 分页从1开始，每页最大数量为100
9. **错误处理**: 所有API调用都应该进行错误处理，并显示用户友好的错误信息

## 11. 更新日志

### 2026-03-03

- 优化阶段状态逻辑：无数据时预览和执行阶段状态显示为 SKIP
- 修复时间字段类型：将时间字段从 long 改为 Long，支持 null 值
- 修复日志界面滚动：刷新时滚动到顶部，显示最新日志
- 新增删除日志接口：支持删除任务执行日志
- 修复任务目录权限：将任务目录路径改为 ./data/tasks

### 2026-02-15

- 优化任务删除逻辑：预览中或执行中的任务默认不允许删除
- 新增批量删除任务接口：删除所有任务时跳过正在执行的任务
- 优化运行日志删除逻辑：删除所有日志文件时跳过最新的不带时间或序号尾缀的日志文件
- 调整页面命名：将预览执行页面改名为任务列表
- 优化任务列表页面信息展示样式：对不同信息做样式区分，适当展示更多信息
- 重构按钮布局：在任务列表中为每个任务添加触发预览、执行、终止按钮
